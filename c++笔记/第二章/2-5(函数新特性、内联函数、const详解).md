...
在 C 和 C++ 中，`const` 关键字用于声明常量，表示某个变量或指针的值不能被修改。`const` 关键字的位置会影响其修饰的对象。以下是 `const char *`、`char const *` 和 `char * const` 三者的区别：

### 1. `const char *`
- **含义**：指向常量字符的指针。
- **解释**：指针本身可以改变，但它所指向的字符是常量，不能通过该指针修改。
- **示例**：
  ```c
  const char *p = "hello";
  p++; // 合法，指针可以改变
...
### 2. `char const *`
- **含义**：指向常量字符的指针。
- **解释**：与 `const char *` 完全相同。`const` 关键字的位置不同，但效果相同。
- **示例**：
  ```c
  char const *p = "hello";
  p++; // 合法，指针可以改变
...
### 3. `char * const`
- **含义**：指向字符的常量指针。
- **解释**：指针本身是常量，不能改变，但它所指向的字符可以被修改。
- **示例**：
  
  ```c
  char * const p = "hello";
  p++; // 非法，指针不能改变
  *p = 'H'; // 合法，可以修改指向的字符（注意：这里修改字符串字面量是未定义行为）
  ```

### 总结
- **`const char *` 和 `char const *`**：两者等价，表示指向常量字符的指针。指针本身可以改变，但指向的字符不能通过该指针修改。
- **`char * const`**：表示指向字符的常量指针。指针本身不能改变，但指向的字符可以被修改。

### 注意事项
- 修改字符串字面量（如 `"hello"`）是未定义行为，即使指针允许修改指向的字符。因此，在实际编程中，通常使用 `const char *` 来指代字符串字面量，以防止意外修改。
