...
int main(void)
{
 ...
 testclass02 = 2000;//这一行产生了一个临时对象, 调用了构造函数（带默认参数）和析构函数
 /*
  *  调用了带参数的构造函数
  调用了拷贝赋值运算符
  调用了析构函数
  ...
  * 用1000构造testclass03对象直接在分配的内存空间分配，没有产生临时对象进行构造
  */
...
myClass operator+(myClass&myclass01,myClass&myclass02)
{
 ...
 return myclass03;
    // 优化
    // return myclass03(myclass01.number01 + myclass02.number01, myclass01.number02 + myclass02.number02)
